...
@JSExportAll
trait CiteCexWriter extends CiteWriter {
...
license#Public Domain."""
  ...
  def writeCexMetadata(name:Option[String], urn:Option[Cite2Urn], license:Option[String], delimiter:String):String = {
  ...
  def writeCitableNode(cn:CitableNode, delim:String):String = {
 ...
 def writeCtsCatalogBlock(vce:Vector[edu.holycross.shot.ohco2.CatalogEntry], delimiter:String = defaultDelim):String = {
    ...
    val header:String = """urn#citationScheme#groupName#workTitle#versionLabel#exemplarLabel#online#lang""".replaceAll("#",delimiter)
 ...
 /* CiteObject */
  ...
  def writeCitePropertiesBlock(ccd:CiteCollectionDef, delim1:String, delim2:String):String = {
    val headerVec:Vector[String] = Vector(
        "#!citeproperties",
        "Property#Label#Type#Authority list".replaceAll("#",delim1)
    )
  ...
  def writeCiteRelationBlock(cr:CiteRelationSet, standalone:Boolean = false, delim1:String = defaultDelim):String = {
    val header:String = {
     if (standalone) writeCexMetadata(Some("CEX Library created by CEXWriter"), Some(Cite2Urn("urn:cite2:cex:TEMPCOLL.TEMPVERSION:TEMP_ID")), Some("Public Domain."), delim1) else ""
    }

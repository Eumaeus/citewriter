...
object HtmlWriter extends CiteWriter {
  ...
  def writeCitableNode(cn:CitableNode):String = {
    ...
    s"""<span class="cite_ohco2_citableNodeContainer" id="${cn.urn}">${passage}${text}${urn}</span>"""
  }

  def writeCitableNodeText(cn:CitableNode):String = {
    s"""<span class="cite_ohco2_citableNodeText">${cn.text}</span>"""
  }

  def writeCtsUrnPassage(u:CtsUrn):String = {
    if (u.passageComponent.size > 0) {
      s"""<span class="cite_ohco1_passageComponent">${u.passageComponent}</span>"""
    } else {
      ""    
    }
  }
  ...
  def writeUrn(u:Urn):String = {
  	s"""<span class="cite_ohco2_citableNodeUrn">${u}</span>"""
  }
